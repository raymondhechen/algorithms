...
// Raymond Chen - HubSpot Coding Challenge (September 1, 2020)
// To run this file: install dependencies using `npm i` or 
// `npm i node-fetch`. Then run `node hubspot.js`

// This solution works by first grouping all the partners into arrays
// containing their own respective countries. Then within each country,
// we want to analyze all of the partner's availabilities and find the
// two consecutive days with the most partners available. To do this,
// we can calculate the number of partners available for every possible
// two-day time slot starting from the first available day among all 
// partners within a country to the last available day among all partners
// within that country. In each time slot, we simply count the number of 
// partners that will be available two consecutive days starting on the 
// day indicated by the time slot value itself. Completing this for all 
// partners, time slots, and countries will derive us the correct solution.
